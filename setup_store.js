...
export interface HMSActions {
  ...
  allocateRoom: (room: Room) => void;
  vacateRoom: (num: number) => void;
  deleteRoom: (num: number) => void;
  log: (message: string, color: string, category: ActivityLog['category']) => void;
  clearLog: () => void;
}
...
export const useHMSStore = create<HMSState & HMSActions>((set, get) => ({
  sidebarOpen: true,
  ...
  fetchAllData: async () => {
    try {
      ...
      const appointments = appointsRaw.map((a: any) => ({
        id: a.appointmentId,
  ...
  dischargePatient: async (id) => {
    const state = get();
    const patientIndex = state.patients.findIndex((p: any) => p.id === id);
    ...
    try {
      ...
      if (roomId !== undefined) get().vacateRoom(roomId);
    } catch(e) {}
  },
  ...
  removeDoctor: async (id) => {
    try {
      ...
      set((state) => ({ doctors: state.doctors.filter((d: any) => d.id !== id) }));
  ...
  toggleDoctorAvailability: (id) => {
    ...
    const state = get();
    const doc: any = state.doctors.find((d: any) => d.id === id);
    if (doc) {
      ...
      set((state) => ({
        doctors: state.doctors.map((d: any) =>
          d.id === id ? { ...d, available: newAvail } : d
        )
      }));
  ...
  removeNurse: async (id) => {
    try {
      ...
      set((state) => ({ nurses: state.nurses.filter((n: any) => n.id !== id) }));
  ...
  cancelAppointment: async (id) => {
    try {
      ...
      set((state) => {
        const updatedAppointments = state.appointments.map((a: any) => 
  ...
  allocateRoom: (room) => {
    set((state) => {
      const existingIndex = state.rooms.findIndex((r: any) => r.num === room.num);
      if (existingIndex >= 0) {
  ...
  vacateRoom: (num) => {
    set((state) => {
      const updatedRooms = state.rooms.map((r: any) => 
        r.num === num 
          ? { ...r, status: 'vacant' as const, patientId: undefined, patientName: undefined, allocatedAt: undefined } 
          : r
      );
      return { rooms: updatedRooms };
    });
  ...
  deleteRoom: (num) => {
    set((state) => ({ rooms: state.rooms.filter((r: any) => r.num !== num) }));
    get().log(\`Room \$\{num} deleted.\`, 'red', 'room');
  }
}));`;
fs.writeFileSync('hms-react/src/store/hmsStore.ts', fileContent);
